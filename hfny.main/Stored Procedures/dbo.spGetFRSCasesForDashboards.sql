...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[spGetFRSCasesForDashboards]  (
...
--DECLARE @sortorder int             = 1
	...
	insert into @tickler
		select pc1id
			  ...
			  ,pc.pcphone + CASE when pc.PCEmergencyPhone is not null and pc.PCEmergencyPhone <> '' then
				    	   ...
				    	   + CASE when pc.PCCellPhone is not null and pc.PCCellPhone <> '' then
							+ CHAR(13) + 'Cell: ' + pc.PCCellPhone ELSE '' END
							 as pcphone
			  ,hvcase.ScreenDate,
			  LTRIM(RTRIM(t.TCFirstName)) + LTRIM(RTRIM(t.TCLastName)) AS tcName
			  ,case
			...
			from hvcase
				...
				left JOIN TCID t ON t.HVCaseFK = HVCase.HVCasePK
	...
	select (select count(*)
		  ...
		  ,case
		   ...
		   tickler.currentLevelPK,
		   tickler.currentLevelName
