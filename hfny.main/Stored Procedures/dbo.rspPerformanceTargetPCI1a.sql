...
-- Description:	gets data for Performance Target report - PCI1a. Primary Care Taker 1 breast feeding
-- exec [rspPerformanceTargetPCI1a] '07/01/2012', '09/30/2012', <<table>>, null
...
-- =============================================
CREATE procedure [dbo].[rspPerformanceTargetPCI1a]
(
    @StartDate      datetime,
    @EndDate		datetime,
...
begin
	...
	as
		(
		select distinct 'PCI1a' as PTCode
			  , c.HVCaseFK
			  , PC1ID
			  , OldID
			  , TCDOB
	...
	select PTCode
			...
			, FormOutOfWindow
			, FormMissing
			, case when (WasBreastFed = 1 and TimeBreastFed >= '05' and FormReviewed = 1 
			  ...
			  , case when FormMissing = 1 then 'Form missing'
						...
						when WasBreastFed = 1 and TimeBreastFed < '05' 
							then 'Breast fed < 6 months'
