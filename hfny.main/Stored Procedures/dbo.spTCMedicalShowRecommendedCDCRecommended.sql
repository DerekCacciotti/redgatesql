...
--red
SELECT  EventDescription, ScheduledEvent, Optional, DueBy, MedicalItemCode, TCItemDate, TCMedicalCreator, DATEADD(DAY,DueBy,@TCDOB) AS estdate, 'past due' AS type FROM dbo.codeDueByDates INNER JOIN dbo.codeMedicalItem ON MedicalItemTitle = ScheduledEvent 
LEFT join dbo.TCMedical ON TCMedicalItem = MedicalItemCode  AND TCIDFK = @TCIDFK
 ...
 UNION
 -- yellow
 (SELECT  EventDescription, ScheduledEvent, Optional, DueBy, MedicalItemCode, TCItemDate, TCMedicalCreator, DATEADD(DAY,DueBy,@TCDOB) AS estdate, 'upcoming' AS type FROM dbo.codeDueByDates INNER JOIN dbo.codeMedicalItem ON MedicalItemTitle = ScheduledEvent 
LEFT join dbo.TCMedical ON TCMedicalItem = MedicalItemCode  AND TCIDFK = @TCIDFK
...
UNION
 (SELECT  EventDescription, ScheduledEvent, Optional, DueBy, MedicalItemCode, TCItemDate, TCMedicalCreator, DATEADD(DAY,DueBy,@TCDOB) AS estdate, 'done' AS type FROM dbo.codeDueByDates INNER JOIN dbo.codeMedicalItem ON MedicalItemTitle = ScheduledEvent 
LEFT join dbo.TCMedical ON TCMedicalItem = MedicalItemCode  AND TCIDFK = @TCIDFK
