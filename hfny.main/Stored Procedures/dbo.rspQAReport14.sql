...
select 
	...
	where   ((h.IntakeDate <= dateadd(M, -1, @LastDayofPreviousMonth)) AND (h.IntakeDate IS NOT NULL))	 		  
			...
			AND codeLevel.LevelName NOT IN ('Level 4', 'Level CO', 'Level CO-term', 'Level TO', 'Level TO-term', 'Level TR', 'Level TR-term')
...
(
		...
		AND SUBSTRING(VisitType, 4, 1) <> '1' -- all regular visits
		GROUP BY HVCaseFK
...
(
	...
	inner join dbo.SplitString(@programfk,',') on h.programfk = listitem
	WHERE SUBSTRING(VisitType, 4, 1) = '1' -- all attempted visits
	GROUP BY HVCaseFK 
...
(
	SELECT HVCaseFK,max(VisitStartTime) VisitStartTime FROM HVLog h 
	inner join dbo.SplitString(@programfk,',') on h.programfk = listitem
	WHERE SUBSTRING(VisitType, 4, 1) <> '1' -- all regular visits
...
VALUES(14 ,'No Home Visits since ' + convert(VARCHAR(12),@Back2MonthsFromAnalysisPoint, 101) + ' for Active Cases Excludes Level TO/CO/TR and Level 4 Cases (N=' + CONVERT(VARCHAR,@numOfALLScreens) + ')' 
