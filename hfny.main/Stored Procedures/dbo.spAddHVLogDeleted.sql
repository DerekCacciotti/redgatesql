...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spAddHVLogDeleted](@HVLogPK int=NULL,
...
AS
IF NOT EXISTS (SELECT TOP(1) HVLogDeletedPK
FROM HVLogDeleted lastRow
WHERE 
@HVLogPK = lastRow.HVLogPK AND
...
@HVLogDeleteDate = lastRow.HVLogDeleteDate AND
@HVLogDeleter = lastRow.HVLogDeleter AND
