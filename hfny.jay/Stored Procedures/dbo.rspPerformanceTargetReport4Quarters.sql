...
-- rspPerformanceTargetReport4Quarters 1 ,'07/01/2012' ,'09/30/2012'
...
-- rspPerformanceTargetReport4Quarters 9, '04/01/2013', '06/30/2013', 471
...
begin
	...
	insert into @tblPTSummary (PTCode, Qtr1PercentageMeetingPT)
		select PTCode
				, round(sum(case when FormMeetsTarget = 1 then 1 else 0 end) / (count(PTCode) * 1.00), 2) as Qtr1PercentageMeetingTarget
	...
	delete from @tblPTDetails
	
	--set @LastRowCount = (select count(*) from @tblPTDetails) - @LastRowCount
...
;	with cteQtr2PercentageMeeting as
		(
			select PTCode
					, round(sum(case when FormMeetsTarget = 1 then 1 else 0 end) / (count(PTCode) * 1.00), 2) as PercentageMeetingTarget
			from @tblPTDetails
			group by PTCode
		)
	update @tblPTSummary
