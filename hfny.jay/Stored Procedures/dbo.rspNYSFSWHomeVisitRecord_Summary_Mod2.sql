...
-- =============================================
CREATE procedure [dbo].[rspNYSFSWHomeVisitRecord_Summary_Mod2](@programfk    varchar(max)    = null,
                                                       @sdate        datetime,
                                                       @edate        datetime,
...
begin
	...
	select rtrim(LastName)+', '+rtrim(FirstName) as WorkerFirstLast
		  ,workerfk
		  ,sum(CaseCount) as casecount
		  ,sum(expvisitcount) as ExpVisitCount
		  ,sum(actvisitcount) as ActVisitCount
		  ,sum(inhomevisitcount) as InHomeVisitCount
		  ,sum(attvisitcount) as AttVisitCount
		  ,sum(minutes) as DirectServiceTimeMinutes
		...
		group by LastName
				...
				,workerfk
	--select workername
	--	  ,rtrim(LastName)+', '+rtrim(FirstName) as WorkerFirstLast
	--	  ,workerfk
	--	  ,pc1id
	--	  ,casecount
