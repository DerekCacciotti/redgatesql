...
-- exec [rspPreIntakeEngagement_test] ',19,','04/01/2013' , '06/30/2013',null,0
-- exec [rspPreIntakeEngagement_test] ',1,','09/01/2010','11/30/2010',null,1

-- =============================================
CREATE procedure [dbo].[rspPreIntakeEngagement_test](@programfk    varchar(max)    = null,
                                                        @sdate        datetime,
                                                        @edate        datetime,                                                        
                                                        @sitefk int             = NULL,
...
IF (@CustomQuarterlyDates = 0)
	BEGIN 
		...
		(
			...
			SELECT HVCasePK, @ContractStartDate , @edate, k.KempeDate as kempeKempeDate, irq.KempeDate as CaseKempeDate
			--count(irq.HVCasePK)
			FROM @tblInitRequiredData irq
			inner join Kempe k on k.HVCaseFK = irq.HVCasePK			
			WHERE 
			--k.KempeDate between '04/01/2013' and '06/30/2013'
			--k.KempeDate between '01/01/2013' and '03/31/2013'
			k.KempeDate between '10/01/2012' and '12/31/2012'
			--k.KempeDate between '04/01/2013' and '06/30/2013'
