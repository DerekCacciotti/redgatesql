...
(
			...
			AND 
			FormDate <= @EndDate	
			group by c.HVCaseFK, cach.PC1HasMedicalProvider
...
cteExpectedForm4TCLessThan6Months
	as
		...
		SELECT 'HD8' as PTCode
			 ...
			 , CASE 
					WHEN (chl6.PC1HasMedicalProvider IS NOT NULL AND chl6.FormDate > tcl6.FormDate AND chl6.FormDate > '01/01/13' AND chl6.PC1HasMedicalProvider = 1 ) -- latest of either TC or CH
							...
							(CASE WHEN (tcl6.PC1HasMedicalProvider IS NOT NULL AND tcl6.PC1HasMedicalProvider = 1) THEN tcl6.FormDate ELSE NULL END )
			 ...
			 , CASE 
					WHEN (chl6.PC1HasMedicalProvider IS NOT NULL AND chl6.FormDate > tcl6.FormDate AND chl6.FormDate > '01/01/13' AND chl6.PC1HasMedicalProvider = 1 ) -- latest of either TC or CH
							...
							(CASE WHEN (tcl6.PC1HasMedicalProvider IS NOT NULL AND tcl6.PC1HasMedicalProvider = 1) THEN 1 ELSE 0 END )
...
cteExpectedForm4TC6MonthsOrOlder
	as
		...
		SELECT 'HD8' as PTCode
			  ...
			  , CurrentLevelName			 				 
			 , CASE 
					WHEN (cach.PC1HasMedicalProvider IS NOT NULL AND cach.FormDate > cafu.FormDate AND cach.FormDate > '01/01/13' AND cach.PC1HasMedicalProvider = 1 ) -- latest CH first preferred
							THEN cach.FormDate -- note: preference is given to the latest CH record first, if there is one
							
							ELSE   -- note: otherwise we will use tcid record's info
							
							(CASE WHEN (cafu.PC1HasMedicalProvider IS NOT NULL AND cafu.PC1HasMedicalProvider = 1) THEN cafu.FormDate ELSE NULL END )
						 					
					END AS FormDate 			 


			  , case 
					when (cach.FormDate IS NOT NULL AND cach.PC1HasMedicalProvider IS NOT NULL) OR (cafu.FormDate IS NOT NULL AND cafu.PC1HasMedicalProvider IS NOT NULL) then 1  
					ELSE
					0
					end 			  
			  as FormReviewed			 
			  ...
			  , case -- Here FormOutOfWindow means that there must be either FU (Due now) or latest CH record in CommonAttribute table for tc >= 6 months
					when (cach.FormDate IS NOT NULL AND cach.PC1HasMedicalProvider IS NOT NULL) OR (cafu.FormDate IS NOT NULL AND cafu.PC1HasMedicalProvider IS NOT NULL) then 0				  
					ELSE
					1
			    end as FormOutOfWindow				 
