...
AS (SELECT count(DISTINCT a.HVCasePK) [8MotherWithOtherChild]
	FROM
		dbo.HVCase AS a
		JOIN dbo.CaseProgram AS b
			ON a.HVCasePK = b.HVCaseFK
		JOIN PC AS c
			ON c.PCPK = a.PC1FK
		JOIN Intake AS d
			ON d.HVCaseFK = a.HVCasePK
		JOIN OtherChild AS oc
			ON oc.FormFK = d.IntakePK AND oc.FormType = 'IN' AND oc.Relation2PC1 = '01'
		INNER JOIN worker fsw
			ON b.CurrentFSWFK = fsw.workerpk
	...
	WHERE
		...
		AND (CASE
			WHEN @SiteFK = 0 THEN
				1
			WHEN workerprogram.SiteFK = @SiteFK THEN
				1
			ELSE
				0
		END = 1)
...
AS (SELECT a.HVCasePK
	...
	FROM
		...
		JOIN Intake AS d
			ON d.HVCaseFK = a.HVCasePK
		JOIN TCID AS tc
			ON tc.HVCaseFK = a.HVCasePK AND tc.TCDOB <= @EndDt
		JOIN CommonAttributes AS caTC
			ON caTC.FormFK = tc.TCIDPK AND caTC.FormType = 'TC'
		INNER JOIN worker fsw
			ON b.CurrentFSWFK = fsw.workerpk
		INNER JOIN workerprogram
			ON workerprogram.workerfk = fsw.workerpk
...
AS (SELECT DISTINCT a.HVCasePK
	...
	FROM
		...
		JOIN workerprogram
			ON workerprogram.workerfk = fsw.workerpk
		LEFT OUTER JOIN CommonAttributes AS ca
			ON ca.FormFK = d.IntakePK AND ca.FormType = 'IN'
		LEFT OUTER JOIN CommonAttributes AS ca1
			ON ca1.FormFK = d.IntakePK AND ca1.FormType = 'IN-PC1'
		LEFT OUTER JOIN CommonAttributes AS ca2
			ON ca2.FormFK = d.IntakePK AND ca2.FormType = 'IN-PC2'
		LEFT OUTER JOIN CommonAttributes AS caOBP
			ON caOBP.FormFK = d.IntakePK AND caOBP.FormType = 'IN-OBP'
		LEFT OUTER JOIN PC AS pc
			ON pc.PCPK = caOBP.PCFK AND pc.Gender = '02'
...
AS (SELECT count(*) [n]
		 ...
		 , sum(CASE
			   ...
			   ELSE
				   0
		   END) [10PrenatalAtEnrolled]
		 , sum(CASE
			   WHEN x.NeedInterpreter = 1 THEN
				   1
			   ELSE
				   0
		   END) [11NeedInterpreter]
	FROM
		x AS x
),
z
AS (SELECT y.*
	...
	FROM
		y
		JOIN MotherWithOtherChildren AS f
			ON 1 = 1
		JOIN TCMedicaid AS g
			ON 1 = 1
