...
-- rspQAReport13_bak 31, 'summary'	--- for summary page
-- rspQAReport13_bak 31			--- for main report - location = 2
-- rspQAReport13_bak null			--- for main report for all locations
-- =============================================


CREATE procedure [dbo].[rspQAReport13_bak](
...
DECLARE @tbl4QAReport13_bak TABLE(
	HVCasePK INT, 
	[PC1ID] [char](13),	
	TCDOB [datetime],
	TCName [varchar](200),
...
DECLARE @tbl4QAReport13_bakDetail TABLE(
	HVCasePK INT, 
	[PC1ID] [char](13),	
	TCDOB [datetime],
...
INSERT INTO @tbl4QAReport13_bakDetail(
	HVCasePK,
	[PC1ID],
	TCDOB,
	FormDueDate,
...
INSERT INTO @tbl4QAReport13_bak
(
	HVCasePK,
	[PC1ID],
	LengthInProgram,
...
SET @numOfALLScreens = (SELECT count(HVCasePK) FROM @tbl4QAReport13_bakDetail)  

DECLARE @numOfActiveIntakeCases INT = 0
SET @numOfActiveIntakeCases = (SELECT count(HVCasePK) FROM @tbl4QAReport13_bak)
...
DECLARE @tbl4QAReport13_bakSummary TABLE(
	[SummaryId] INT,
	[SummaryText] [varchar](200),
	[SummaryTotal] [varchar](100)
)

INSERT INTO @tbl4QAReport13_bakSummary([SummaryId],[SummaryText],[SummaryTotal])
...
SELECT * FROM @tbl4QAReport13_bakSummary	
