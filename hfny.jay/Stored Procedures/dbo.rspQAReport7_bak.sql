...
-- rspQAReport7_bak 31, 'summary'	--- for summary page
-- rspQAReport7_bak 31			--- for main report - location = 2
-- rspQAReport7_bak null			--- for main report for all locations
-- =============================================


CREATE procedure [dbo].[rspQAReport7_bak](
...
DECLARE @tbl4QAReport7_bakDetail TABLE(
	HVCasePK INT, 
	[PC1ID] [char](13),	
	TCDOB [datetime],
...
INSERT INTO @tbl4QAReport7_bakDetail(
	HVCasePK,
	[PC1ID],
	TCDOB,
	FormDueDate,
...
UPDATE @tbl4QAReport7_bakDetail SET TCName = '' WHERE TCName IS NULL



-- Equivelent to csrForm6 in foxpro	
--SELECT DISTINCT HVCasePK  FROM @tbl4QAReport7_bakDetail
SELECT * FROM @tbl4QAReport7_bakDetail  -- Equivelent to csrForm6 in foxpro
ORDER BY HVCasePK
 
--- rspQAReport7_bak 31 ,'summary'

DECLARE @tbl4QAReport7_bakDetailEF TABLE(
	HVCasePK INT,
	[PC1ID] [char](13),
	TCDOB [datetime],
	FormDueDate [datetime],
...
-- FROM @tbl4QAReport7_bakDetail qa1 --, @tbl4DueByDates 
...
SELECT 
 ...
 FROM @tbl4QAReport7_bakDetail qa1
 inner join codeduebydates on scheduledevent = 'ASQ' AND AgeInDays <= DueBy  
...
INSERT INTO @tbl4QAReport7_bakDetailEF(
	HVCasePK,
	[PC1ID],
	TCDOB,
	FormDueDate,
 ...
 SELECT * FROM @tbl4QAReport7_bakDetailEF
 --WHERE  OutOfWindow = 1

	
--- rspQAReport7_bak 31 ,'summary'

--SELECT * FROM @tbl4QAReport7_bakDetail qa7
...
--	SET @numOfALLScreens = (SELECT count(HVCasePK) FROM @tbl4QAReport7_bakDetail
--	WHERE AgeInDays > 122 AND (tcdod > @LastDayofPreviousMonth OR tcdod IS NULL))

--	DECLARE @numOfActiveIntakeCases INT = 0
--	SET @numOfActiveIntakeCases = (
--	SELECT count(HVCasePK) FROM @tbl4QAReport7_bakDetail
...
--DECLARE tbl4QAReport7_bakSummary TABLE(
--	[SummaryId] INT,
--	[SummaryText] [varchar](200),
--	[SummaryTotal] [varchar](100)
--)

--INSERT INTO tbl4QAReport7_bakSummary([SummaryId],[SummaryText],[SummaryTotal])
...
--	SELECT * FROM @tbl4QAReport7_bakSummary	
